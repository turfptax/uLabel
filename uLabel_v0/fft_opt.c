...
void fft8_real(float *Xr, float *Yr, float *Yi)
{
	const float sn14 = 1;
	const float cs14 = 0;
	const float sn18 = 0.707106772;
	const float cs18 = 0.707106772;
	const float sn38 = 0.707106772;
	const float cs38 = -0.707106772;
	
	float x0p4r = Xr[0] + Xr[4];
	float x0m4r = Xr[0] - Xr[4];
	float x2p6r = Xr[2] + Xr[6];
	float x2m6r = Xr[2] - Xr[6];
	float x1p5r = Xr[1] + Xr[5];
	float x1m5r = Xr[1] - Xr[5];
	float x3p7r = Xr[3] + Xr[7];
	float x3m7r = Xr[3] - Xr[7];
	
	Yr[0] = x0p4r + x2p6r + x1p5r + x3p7r;
	Yi[0] = 0;
	Yr[1] = x0m4r + x2m6r*cs14 + (x1m5r + x3m7r*cs14)*cs18 - x3m7r*sn14*sn18;
	Yi[1] = x2m6r*sn14 + x3m7r*sn14*cs18 + (x1m5r + x3m7r*cs14)*sn18;
	Yr[2] = x0p4r - x2p6r + (x1p5r-x3p7r)*cs14;
	Yi[2] = (x1p5r-x3p7r)*sn14;
	Yr[3] = x0m4r - x2m6r*cs14 + (x1m5r - x3m7r*cs14)*cs38 + x3m7r*sn14*sn38;
	Yi[3] = - x2m6r*sn14 - x3m7r*sn14*cs38 + (x1m5r - x3m7r*cs14)*sn38;
	Yr[4] = x0p4r + x2p6r - x1p5r - x3p7r;
	Yi[4] = 0;
	Yr[5] = x0m4r + x2m6r*cs14 - (x1m5r + x3m7r*cs14)*cs18 + x3m7r*sn14*sn18;
	Yi[5] = x2m6r*sn14 - x3m7r*sn14*cs18 - (x1m5r + x3m7r*cs14)*sn18;
	Yr[6] = x0p4r - x2p6r - (x1p5r-x3p7r)*cs14;
	Yi[6] = -(x1p5r-x3p7r)*sn14;
	Yr[7] = x0m4r - x2m6r*cs14 - (x1m5r - x3m7r*cs14)*cs38 - x3m7r*sn14*sn38;
	Yi[7] = -x2m6r*sn14 + x3m7r*sn14*cs38 - (x1m5r - x3m7r*cs14)*sn38;
}
